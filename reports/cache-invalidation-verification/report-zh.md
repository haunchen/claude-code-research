...
不是所有快取失效都一樣嚴重。有些會摧毀整個前綴，有些只影響訊息層。以下是根據 CLI v2.1.85 原始碼分析的完整表格。

### 完整前綴失效（tools + system + 所有訊息重建）

| 操作 | 為什麼會失效 | 在 200k 上下文的成本 | 證據來源 |
|------|------------|--------------------|---------|
| **加入一個非延遲工具** | 工具前綴在位置 0 改變 | ~$3.75（125% 寫入） | 內嵌文件 |
| **MCP server 重啟且工具描述改變** | 工具 schema 的位元組改變 | ~$3.75 | 工具序列化分析 |
| **對話途中切換模型** | 快取金鑰包含 model ID | ~$3.75 | API 設計 |
| **從 standard 切換到 tool-search 模式** | 工具陣列組成改變（ToolSearch 加入，延遲工具移除） | ~$3.75 | 原始碼 |

### 系統提示失效（system + 訊息重建，tools 保留快取）

| 操作 | 為什麼會失效 | 在 200k 上下文的成本 | 證據來源 |
|------|------------|--------------------|---------|
| **日期翻日（currentDate 改變）** | 動態區內容變了 | ~$3.00 | 系統提示分析 |
| **對話途中編輯 CLAUDE.md** | 系統提示內容變了 | ~$3.00 | 系統提示重建 |
| **兩輪之間 git 狀態改變** | 動態區包含 git 資訊 | ~$3.00 | 系統提示分析 |
| **首次出現非延遲 MCP 工具** | 觸發快取策略從 global 切到 org scope | ~$3.00 | Z57() 分支邏輯 |

### 純訊息失效（只有新訊息未被快取）

| 操作 | 為什麼會失效 | 成本 | 證據來源 |
|------|------------|------|---------|
| **正常的對話輪次** | 新訊息加入，斷點往前滑 | 很小（只寫入新內容） | kVY() 滑動視窗 |
| **工具回傳的內容** | 工具輸出成為訊息的一部分 | 與輸出大小成正比 | 正常運作 |

### 完整上下文重建（所有內容重新處理）

| 操作 | 為什麼會失效 | 在 200k 上下文的成本 | 證據來源 |
|------|------------|--------------------|---------|
| **上下文壓縮（auto-compact）** | 訊息被摘要改寫，前綴改變 | ~$3.75 + 連鎖反應風險 | 壓縮分析 |
| **Session resume（關閉後重開）** | 整個 request 從持久化狀態重建 | ~$3.75 | Session 架構 |
| **快取 TTL 過期（閒置超過 5 分鐘或 1 小時）** | 伺服器清除快取條目 | ~$3.75（下次請求時） | XU() TTL 邏輯 |

### 不影響快取

| 操作 | 為什麼安全 | 證據來源 |
|------|----------|---------|
| **透過 ToolSearch 發現 MCP 工具** | defer_loading 把工具排除在前綴之外 | 本報告 |
| **使用 Skill tool** | Skill 內容附加在訊息末端，不改變 tools 或 system | Skill 架構 |
| **讀取檔案（Read tool）** | 輸出進入訊息，斷點正常滑動 | 正常運作 |
| **連續多次 ToolSearch 呼叫** | 每次發現都是加入 defer_loading 的工具，前綴不變 | defer_loading 機制 |

### 成本估算公式

任何快取失效事件的成本，取決於重建了什麼：

```
完整前綴重建成本    = (tools_tokens + system_tokens + messages_tokens) × 寫入費率
系統重建成本       = (system_tokens + messages_tokens) × 寫入費率
訊息重建成本       = (new_message_tokens) × 寫入費率

其中：
  寫入費率 = $1.25/MTok（5 分鐘 TTL）或 $2.00/MTok（1 小時 TTL）
  讀取費率 = $0.10/MTok（快取命中）

每次失效的淨成本 = 重建成本 - (原本會被讀取的量 × 讀取費率)
               ≈ context_tokens × (寫入費率 - 讀取費率)
```

200k 上下文、1 小時 TTL 的情況：
- 完整重建：200k × $2.00/MTok = $0.40（寫入成本）
- 如果命中快取只需付：200k × $0.10/MTok = $0.02（讀取成本）
- **一次快取失效的淨成本：約 $0.38**

500k 上下文、1 小時 TTL 的情況：
- **一次快取失效的淨成本：約 $0.95**

這些數字是單次失效的成本。在一段長對話中如果頻繁觸發失效（例如 CLAUDE.md 自動儲存、兩輪之間有 git 操作），成本會疊加。
